-- FIXED DATABASE SCRIPT - RFP Win Platform
-- This version handles different customer table structures

-- ========================================
-- STEP 1: CHECK CUSTOMERS TABLE STRUCTURE
-- ========================================

-- First, let's see what your customers table looks like
...
-- Show sample customer data to understand the ID field
SELECT 'SAMPLE CUSTOMER DATA' as info;
SELECT * FROM public.customers LIMIT 2;

-- ========================================
-- STEP 2: CREATE MISSING TABLES (FIXED)
-- ========================================

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Create RFPs table (using 'id' instead of 'customer_id' to match your customers table)
...
BEGIN
    -- Try to find customer by email (most common scenarios)
    SELECT * INTO customer_record FROM public.customers 
    WHERE email = 'algomind6@gmail.com' 
    OR email ILIKE '%algomind%'
    OR email ILIKE '%gmail%'
    LIMIT 1;
    
    -- If no specific customer found, get the first customer
    IF customer_record.id IS NULL THEN
        SELECT * INTO customer_record FROM public.customers LIMIT 1;
    END IF;
    
    IF customer_record.id IS NOT NULL THEN
        -- Use the customer ID (whether it's 'id' or another field)
        ...
        VALUES 
        (customer_id_value, COALESCE(customer_record.email, 'test@example.com'), 'rfp_analysis', rfp_id_1, '{"filename": "cloud-migration-rfp.pdf", "file_size": 2048576, "analysis_duration": 45}', NOW() - INTERVAL '30 days'),
        (customer_id_value, COALESCE(customer_record.email, 'test@example.com'), 'proposal_generation', rfp_id_1, '{"rfp_title": "Cloud Infrastructure Migration", "proposal_length": 4500, "generation_time": 120}', NOW() - INTERVAL '28 days'),
        (customer_id_value, COALESCE(customer_record.email, 'test@example.com'), 'rfp_analysis', rfp_id_2, '{"filename": "software-dev-rfp.pdf", "file_size": 1536000, "analysis_duration": 38}', NOW() - INTERVAL '20 days'),
        (customer_id_value, COALESCE(customer_record.email, 'test@example.com'), 'proposal_generation', rfp_id_2, '{"rfp_title": "Enterprise Software Development", "proposal_length": 3200, "generation_time": 95}', NOW() - INTERVAL '18 days'),
        (customer_id_value, COALESCE(customer_record.email, 'test@example.com'), 'rfp_analysis', rfp_id_3, '{"filename": "marketing-platform-rfp.pdf", "file_size": 1024000, "analysis_duration": 32}', NOW() - INTERVAL '10 days'),
        (customer_id_value, COALESCE(customer_record.email, 'test@example.com'), 'proposal_generation', rfp_id_3, '{"rfp_title": "Digital Marketing Platform", "proposal_length": 2800, "generation_time": 85}', NOW() - INTERVAL '8 days');

        RAISE NOTICE 'SUCCESS: Added data for customer ID % with email %', customer_id_value, customer_record.email;
    ELSE
        RAISE NOTICE 'ERROR: No customers found in the table. Please add a customer first.';
    END IF;
END $$;
...
SELECT '🎉 DATABASE SETUP COMPLETE! Check your dashboard now!' as final_status; 